#!/bin/bash
# Order API stress test for Java service using wrk with database writes
# This tests the BASELINE performance with MySQL transactions

set -e

# Configuration
URL="${1:-http://localhost:8081/api/v1/orders}"
THREADS="${2:-12}"
CONNECTIONS="${3:-100}"
DURATION="${4:-30s}"

echo "=========================================="
echo "Java Order API Stress Test (Baseline)"
echo "=========================================="
echo ""
...
if [ "$HEALTH_CHECK" != "200" ]; then
    echo "WARNING: Server is not responding at http://localhost:8081/health"
    echo ""
...
echo "  Example: ./benchmark_orders.sh http://localhost:8081/api/v1/orders 24 200 60s"
echo ""
