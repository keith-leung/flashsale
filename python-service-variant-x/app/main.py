...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Flash Sale Service (Variant X - Redis-Only)", extra={"service": "flash-sale-python-variant-x"})
    await cache.connect()
    yield
    # Shutdown
    logger.info("Shutting down Flash Sale Service (Variant X)", extra={"service": "flash-sale-python-variant-x"})
    await cache.close()
