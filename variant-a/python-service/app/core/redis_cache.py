...
from app.services.adaptive_inventory import AdaptiveInventoryManager
...
class RedisCache:
    def __init__(self):
        ...
        self.adaptive_inventory: Optional[AdaptiveInventoryManager] = None
    ...
    async def connect(self):
        if not self.client:
            ...
            # Initialize adaptive inventory manager
            self.adaptive_inventory = AdaptiveInventoryManager(self.client)
            await self.adaptive_inventory.initialize()
            logger.info("Adaptive Inventory Manager initialized")
    ...
    async def queue_order(self, order_data: Dict[str, Any], stream_name: str = "order_queue"):
        ...
        # Serialize list/dict fields to JSON for Redis compatibility
        import json
        serialized_data = {}
        ...
        await self.client.xadd(stream_name, serialized_data)

    def get_adaptive_inventory(self) -> AdaptiveInventoryManager:
        """Get adaptive inventory manager."""
        if not self.adaptive_inventory:
            raise RuntimeError("Adaptive inventory manager not initialized")
        return self.adaptive_inventory

    # ============================================================
    # Variant A: Flash Sale SKU Methods (with campaign association)
    # ============================================================

    def _get_campaign_sku_limit_key(self, flash_sale_id: Union[str, UUID], sku_id: Union[str, UUID]) -> str:
        """Get Redis key for campaign SKU limit counter (Variant A)."""
        return f"fs:{str(flash_sale_id)}:sku:{str(sku_id)}:limit"

    async def set_campaign_sku_limit(self, flash_sale_id: Union[str, UUID], sku_id: Union[str, UUID], limit: int):
        """Set campaign SKU limit counter (Variant A)."""
        if not self.client:
            return

        key = self._get_campaign_sku_limit_key(flash_sale_id, sku_id)
        await self.client.set(key, limit)

    async def get_campaign_sku_limit(self, flash_sale_id: Union[str, UUID], sku_id: Union[str, UUID]) -> Optional[int]:
        """Get current campaign SKU limit remaining (Variant A)."""
        if not self.client:
            return None
