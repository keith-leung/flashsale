...
from app.services.order_logger import get_order_logger
...
async def _create_order_variant_x(
    ...
    """
    Variant A: Adaptive inventory with 2-tier batching (BATCH MODE + DIRECT MODE).
    Used when SKU is in active flash sale campaign.
    Achieves 99.6%+ network I/O reduction through local caching.
    """
    start_time = time.perf_counter()
    logger.info(f"Using Variant A (Adaptive Inventory) for order {order_number}, flash sale {flash_sale_id}")

    # Extract customer info for logging
    customer_email = order_data.customer_email
    customer_name = order_data.customer_name or "Unknown"
    sku_ids = [str(item.sku_id) for item in order_data.line_items]
    quantities = [item.quantity for item in order_data.line_items]

    # Step 1: Reserve from campaign limit (still use campaign-level counter)
    ...
    if campaign_remaining < 0:
        ...
        # Log failed attempt
        await order_logger.log_order_failure(
            order_number=order_number,
    ...
    reserved_skus = []
    adaptive_manager = redis_cache.get_adaptive_inventory()
    inventory_modes = []  # Track which mode was used for each SKU

    try:
        for item_data in order_data.line_items:
            sku_id = str(item_data.sku_id)

            # Get adaptive inventory service for this SKU
            adaptive_service = await adaptive_manager.get_service(flash_sale_id, sku_id)

            # Reserve items one by one using adaptive batching
            for _ in range(item_data.quantity):
                reserved = await adaptive_service.reserve_item()
                ...
                if not reserved:
                    ...
                    await order_logger.log_order_failure(
                        ...
                        quantities=quantities,
                        campaign_id=flash_sale_id,
                        mode=adaptive_service.current_mode,
                        duration_ms=duration_ms,
                        reason=f"SKU {sku_id} sold out"
                    )
            ...
            inventory_modes.append(adaptive_service.current_mode)

        # Determine overall mode (use DIRECT if any SKU used DIRECT mode)
        overall_mode = "DIRECT" if "DIRECT" in inventory_modes else "BATCH"
        ...
        # Step 4: Calculate duration and log success
        duration_ms = (time.perf_counter() - start_time) * 1000

        # Log successful order
        await order_logger.log_order_success(
            order_number=order_number,
            customer_email=customer_email,
            customer_name=customer_name,
            sku_ids=sku_ids,
        ...
        logger.info(
            f"Order {order_number} reserved successfully (Variant A), mode={overall_mode}, "
            f"queued for persistence, duration={duration_ms:.2f}ms"
        )

        # Return immediate success response (build full OrderResponse for API consistency)
        from decimal import Decimal
        from datetime import datetime

        # Calculate prices from SKU metadata
        line_items_response = []
        subtotal = Decimal('0')
        for item in order_data.line_items:
            sku_id = str(item.sku_id)
            meta = sku_metadata.get(sku_id, {})
            unit_price = Decimal(meta.get("flash_price", meta.get("price", "99.99")))
            total_price = unit_price * item.quantity
            subtotal += total_price

            line_items_response.append({
                "id": str(UUID(bytes=os.urandom(16))),  # Temporary ID
                "sku_id": sku_id,
                "quantity": item.quantity,
                "unit_price": str(unit_price),
                "total_price": str(total_price),
                "product_name": meta.get("name", "Product"),
                "sku_code": meta.get("sku_code", "SKU"),
                "created_at": datetime.now().isoformat()
            })

        total_amount = subtotal + Decimal(order_data.tax_amount or 0) + Decimal(order_data.shipping_amount or 0)

        return ResponseDTO(
            status=201,
            message="Order created successfully (flash sale)",
            data={
                "id": str(UUID(bytes=os.urandom(16))),  # Temporary ID until persisted
                "order_number": order_number,
                "customer_email": order_data.customer_email,
                "customer_name": order_data.customer_name or "",
                "subtotal": str(subtotal),
                "tax_amount": str(order_data.tax_amount or Decimal('0')),
                "shipping_amount": str(order_data.shipping_amount or Decimal('0')),
        ...
        # Rollback all reservations on unexpected error
        for rollback_sku_id, rollback_qty in reserved_skus:
            await redis_cache.release_sku_inventory(rollback_sku_id, rollback_qty)
        await redis_cache.release_campaign_inventory(flash_sale_id, total_quantity)
