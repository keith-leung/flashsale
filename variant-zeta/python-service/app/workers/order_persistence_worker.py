"""Order persistence worker - Variant Zeta (Redis-First Architecture) - FIXED."""
...
class OrderPersistenceWorker:
    ...
    async def process_order(self, db: AsyncSession, order_data: Dict[str, Any]):
        ...
        if not sku:
            logger.error(f"Worker {self.worker_id}: SKU {sku_id} not found")
            return False
        
        # Get campaign (if exists)
        campaign = None
        if 'spu_id' in order_data and order_data['spu_id']:
            result = await db.execute(
        ...
        # FIXED: Update inventory (decrement quantity)
        await db.execute(
            update(Inventory)
        ...
        logger.debug(f"Worker {self.worker_id}: Processed order {order_id} (SKU: {sku_id}, Qty: {quantity})")
        return True
    
    async def sync_campaign_cache(self, db: AsyncSession, redis, spu_id: str):
        """Sync campaign data from database to Redis (write-through)."""
        try:
            # Fetch latest campaign data from DB
            result = await db.execute(
                select(FlashSaleCampaign).where(FlashSaleCampaign.spu_id == spu_id)
            )
            campaign = result.scalar_one_or_none()
            
            if campaign:
                # Update Redis with latest campaign data
                redis_key = f'campaign:{spu_id}'
                await redis.hset(redis_key, mapping={
                    'id': campaign.id,
                    'spu_id': campaign.spu_id,
                    'name': campaign.name,
                    'description': campaign.description or '',
                    'total_sale_limit': str(campaign.total_sale_limit),
                    'sold_quantity': str(campaign.sold_quantity),
                    'start_time': str(campaign.start_time),
                    'end_time': str(campaign.end_time),
                    'flash_price': str(campaign.flash_price) if campaign.flash_price else '',
                    'max_quantity_per_customer': str(campaign.max_quantity_per_customer),
                    'status': campaign.status,
                    'is_active': 'true' if campaign.is_active else 'false'
                })
                
                # Set TTL (60 seconds as per design)
                await redis.expire(redis_key, 60)
                
                logger.debug(f"Worker {self.worker_id}: Synced campaign {spu_id} to Redis")
        except Exception as e:
            logger.error(f"Worker {self.worker_id}: Failed to sync campaign cache: {e}")
    
    async def flush_batch(self, batch: list[Dict[str, Any]]):
        """Persist a batch of orders to database with cache sync."""
        if not batch:
            return
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                for order_data in batch:
                    ...
                    # Track SPU IDs for cache sync
                    if 'spu_id' in order_data and order_data['spu_id']:
                        spu_ids.add(order_data['spu_id'])
                
                await db.commit()
                logger.info(f"Worker {self.worker_id}: Persisted batch of {len(batch)} orders")
                
                # FIXED: Sync campaign cache (write-through)
                for spu_id in spu_ids:
                    await self.sync_campaign_cache(db, redis, spu_id)
    ...
    async def start(self):
        ...
        # FIXED: Monitor all queues (order_queue:{spu_id}:worker_{0-4})
        queue_prefix = 'order_queue:'
        
        while self.running:
            try:
                # Try to pop from all queues
                # For now, use simple orders_queue (will update to per-SPU queues)
                ...
                if not order_data:
                    logger.error(f"Worker {self.worker_id}: Order {order_id} not found in Redis")
                    continue
