...
DB_CHECK=$(docker exec flash-mariadb-y mysql -usyracuse -pOrange_315_Forever! -e "SELECT 1" orange315 2>/dev/null)
if [ $? -eq 0 ]; then
...
if [ "$ALL_OK" = true ]; then
    ...
    echo "  • Run unit tests: docker exec flash-python-y python -m pytest"
    ...
    echo -e "${YELLOW}========================================${NC}"
    echo ""
    echo "To fix, run: docker compose up -d"
