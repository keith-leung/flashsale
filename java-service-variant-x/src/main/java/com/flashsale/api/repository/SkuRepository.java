...
public interface SkuRepository extends JpaRepository<Sku, UUID> {
    ...
    // Variant X: Batch SKU loading for Redis optimization
    @Query("SELECT s FROM Sku s LEFT JOIN FETCH s.inventory LEFT JOIN FETCH s.spu WHERE s.id IN :ids")
    List<Sku> findAllByIdWithInventoryAndSpu(@Param("ids") List<UUID> ids);
}
